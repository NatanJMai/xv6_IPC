...
printf(int fd, char *s, ...)
{
  ...
  1e:	e8 77 03 00 00       	call   39a <write>
...
forktest(void)
{
  ...
  5c:	75 05                	jne    63 <forktest+0x3a>
      exit();
  5e:	e8 17 03 00 00       	call   37a <exit>
{
  int n, pid;
  ...
  90:	83 c4 10             	add    $0x10,%esp
    exit();
  93:	e8 e2 02 00 00       	call   37a <exit>
  }
  
  for(; n > 0; n--){
    if(wait() < 0){
  98:	e8 e5 02 00 00       	call   382 <wait>
  ...
  b0:	83 c4 10             	add    $0x10,%esp
      exit();
  b3:	e8 c2 02 00 00       	call   37a <exit>
  if(n == N){
    printf(1, "fork claimed to work N times!\n", N);
    exit();
  }
  ...
  if(wait() != -1){
  c2:	e8 bb 02 00 00       	call   382 <wait>
  ...
  db:	83 c4 10             	add    $0x10,%esp
    exit();
  de:	e8 97 02 00 00       	call   37a <exit>
  }
...
main(void)
{
 ...
 109:	e8 1b ff ff ff       	call   29 <forktest>
  exit();
 10e:	e8 67 02 00 00       	call   37a <exit>
...
gets(char *buf, int max)
{
 ...
 233:	50                   	push   %eax
 234:	6a 00                	push   $0x0
 236:	e8 57 01 00 00       	call   392 <read>
...
stat(char *n, struct stat *st)
{
 ...
 299:	e8 1c 01 00 00       	call   3ba <open>
